...
type
  TfrmScannforr = class(TForm)
    ...
    procedure LimpaRelaçãoCoordenadasPixel;
...
begin
  ...
  if qryArea_geral.State = dsEdit then
    ...
    begin
      showmessage('Para ativar a calibragem de tela é preciso colocar ' +
                  'o registro em modo de edição clicando na barra de navegação');
    end;
...
begin
  ...
  if MessageDlg ('Recalcula posições da tela a partir das coordenadas medidas?'
                 , mtWarning,[mbYes, mbNo], 0) = mrYes then
  if MessageDlg ('***** Atenção, o recálculo das posições da tela pode exigir ' +
                 'que seja feito um novo ajuste de tela para essa medição, ' +
                 'Continua?'
                 , mtWarning,[mbYes, mbNo], 0) = mrYes then
...
begin
  ...
  if MessageDlg ('***** Atenção, recupera os valores padrão dos parâmetros de ' +
                 'massa seca e altura mínima para pastejo?'
                 , mtWarning,[mbYes, mbNo], 0) = mrYes then
    begin
...
begin
  ...
  if (length(trim(edAlturaEquipamento.Text)) = 0) or
    ...
    begin
      showmessage('Altura do equipamento não informada');
  ...
  if (length(trim(edAlturaPlanta.Text)) = 0) or
    ...
    begin
      showmessage('Altura da planta não informada');
  ...
  if (length(trim(dbeDiasPastejo.Text)) = 0) or
    ...
    begin
      showmessage('Quantidade de dias que vai durar o pastejo na lotação ');
  ...
  if (length(trim(dbePesoMedioCabeca.Text)) = 0) or
    ...
    begin
      showmessage('Peso médio (kg) por cabeça dos animais que irão compor a ' +
                  'lotação não informado');
  ...
  begin
    if (length(trim(edms.Text)) = 0) or
      ...
      begin
        showmessage('Massa Seca em kg/m² não informada');
        edms.SetFocus;
        qryMedicoes_capa_scann.CancelUpdates;
        exit;
      end;
    ...
    if (length(trim(edAlturaMassaSeca.Text)) = 0) or
      ...
      begin
        showmessage('Altura da forragem de amostragem da massa seca não informada');
    ...
    if (length(trim(edaltMinima.Text)) = 0) or
      ...
      begin
        showmessage('Altura mínima de pastejo não informada');
...
begin
  ...
  if (length(trim(qryArea_geral_scann.FieldByName('relacao_dla_pixel').AsString)) = 0) or
    ...
    begin
      showmessage('relação do dla/pixel não informada, entre em Áreas de Piquetes ' +
  ...
  if (length(trim(qryArea_geral_scann.FieldByName('relacao_dlo_pixel').AsString)) = 0) or
    ...
    begin
      showmessage('relação do dlo/pixel não informada, entre em Áreas de Piquetes ' +
                  'para cadastrar esse dado');
      exit;
    end;
...
begin
  ...
  try
    ...
    on E : Exception do
    begin
      ShowMessage('erro ao apagar as medições em medicoes_escaneamentos_tb : '+E.Message);
      exit;
    end;
  end;
...
begin
  ...
  // verifica se existe a média da altura apurada e gravada por área
  qryGeral2.Close;
  qryGeral2.SQL.Text :=
...
begin
  ...
  // apaga a média da altura apurada e gravada por área
  qryGeral2.Close;
  qryGeral2.SQL.Text :=
  ...
  try
    ...
    on E : Exception do
    begin
      ShowMessage('erro ao apagar as médias de altura na altura_media_scan_tb : '+E.Message);
      exit;
    end;
  end;
...
begin
  ...
  // verifica percentual de medições fora do piquete
...
{      '         where m.ytop = al.ytop ' +
      ...
      if qryGeral.FieldByName('semPiquete').AsInteger > 0 then
        showmessage('Existe ' + qryGeral.FieldByName('semPiquete').AsString +
                    ' medições que ficaram fora de algum piquete');
      ...
      if xperc > 10.00 then
        begin
          showmessage(currTOstr(xperc) + '% das medições estão fora de algum piquete, '+
                      'faça o ajuste dos piquetes ou ajuste dos pontos na tela devidamente '+
                      'ou tente gerar os pontos de tela novamente clicando no botão Gera' +
                      ' Pontos Tela');
          //exit;
        end;
    end;
  ...
  // calcula altura média da forragem por piquete pelo escaneamento
  qryGeral.Close;
  qryGeral.SQL.Text :=
...
//  '           and (' + currTOstr(xaltura_equipamento) + ' - m.distancia) > 0' +
  ...
  if qryGeral.RecordCount > 0 then
    begin
      ...
      if qryGeral2.RecordCount = 0 then
        if not cktodosPiquetes.Checked then
          begin
            showmessage('Na primeira vez que está gerando os resultados '+
                        'é necessário que sejam processados todos os piquetes, '+
      ...
      try
        ...
        on E : Exception do
        begin
          ShowMessage('erro ao apagar as médias de altura na altura_media_scan_tb'+
    ...
    begin
      // procura a média da altura apurada e gravada por área
      qryGeral.Close;
      qryGeral.SQL.Text :=
  ...
  if qryGeral.RecordCount = 0 then
    begin
      pnProcessa.Visible := false;
      showmessage('Não foi encontrada nenhuma medição');
      exit;
    end;
  ...
  for i:=1 to qryGeral.RecordCount do
    begin
      ...
      if qryMedicoes_escaneamentos.RecordCount > 0 then
        begin
          ...
          try
            ...
            on E : Exception do
            begin
              ShowMessage('erro ao incluir as médias de altura na altura_media_scan_tb : '+E.Message);
              exit;
            end;
          end;
        end;

      // Monta gráfico volume planta *****
      xvolumePlanta :=
      ...
      // Monta gráfico massa seca pastejo kg *****
      xMassaPastejoArea :=
      ...
      // Monta gráfico massa seca crescimento kg por cm
      ...
      // Monta gráfico altura extrato planta pastejo
      ...
      // Monta gráfico volume extrato planta pastejo
      ...
      // Monta gráfico massa seca do extrato de pastejo disponível kg
      ...
      begin
        mmdisponivel.Lines.Add(
          qryGeral.FieldByName('nome_area').AsString +
          ' - sem extrato de massa seca pastejável, está zerada ou negativa, ' +
          'veja no gráfico de massa seca pastejável disponível'
        );
      end
      ...
      begin
        ...
        mmdisponivel.Lines.Add(
          ...
          '% - peso médio por cabeça em kg: ' +
          qryMedicoes_capa_scann.FieldByName('peso_medio_cabeca_kg').AsString
        );

        mmdisponivel.Lines.Add(
          '     ***  lotação: ' + xqtdeAnimaisLotacao.ToString + ' animais em ' +
          formatCurr('###,###,##0.00',
                (qryGeral.FieldByName('tamanhoArea').AsCurrency / 10000)) +
          ' ha - massa seca pastejável: ' +
          formatCurr('###,###,##0.00',
                (xMassaPastejoExtratoArea /
                  qryGeral.FieldByName('tamanhoArea').AsCurrency ) * 10000) +
          ' kg/ha - total de massa seca pastejável do piquete: ' +
      ...
      // Monta gráfico massa seca disponível kg
      xMassaSecaArea :=
          (xmassa_seca * idxMassa) * qryGeral.FieldByName('tamanhoArea').AsCurrency;
...
begin
  ...
  //result := trunc((altura_equipamento - distancia) * idxPlanta);
  // calcula altura média da forragem por piquete
  qryGeral.Close;
  qryGeral.SQL.Text :=
            ...
            if (xseq_area_ant <> qryGeral.FieldByName('seq_area').AsInteger) then
              ...
              begin
                ...
                mmCrescimentoForr.Lines.Add(
                  ...
                  FormatDateTime('dd/mm/yyyy', xdataAnt) + ' até ' +
...
'   and seq_variedade = :seq_variedade';
    ...
    on E : Exception do
    begin
      ShowMessage('erro na leitura da variedade da área de piquetes : '+E.Message);
      exit;
    end;
  end;
  ...
  if qryGeral.RecordCount = 0 then
    begin
      showmessage('variedade da área de piquetes não encontrada - seq_empresa ' +
  ...
  if qryGeral2.RecordCount = 0 then
    begin
      showmessage('Todos as coordenadas com altitude do terreno ' +
                  'já estão no sistema, para essa medição');
      exit;
    end;

  if MessageDlg ('Deseja fazer a importação de altitudes do solo do Google Maps ' +
                 'para ' + qryGeral2.RecordCount.ToString + ' medições?'
...
begin
  ...
  if length(trim(memo2.Text)) = 0 then
    begin
      showmessage('Google maps não retornou altitude do solo');
      str.Free;
      exit
    end;
  ...
  try
    ...
    showmessage('Valor da altitude retornada do gooble maps está inválido = ' +
                xretorno + ' ------ ' + memo2.Text);
    str.Free;
    exit;
  end;
...
begin
  ...
  if (qryAreas.State <> dsedit) then
    begin
      showmessage('Para ativar a demarcação de piquete é preciso colocar ' +
                  'o registro em modo de edição clicando na barra de navegação');
      exit;
    end;
...
begin
  ...
  if qryGeral2.RecordCount = 0 then
    begin
      if MessageDlg ('Os resultados ainda não foram gerados, Tem certeza que '+
                     'deseja eliminar os lançamentos escaneados?'
                     , mtWarning,[mbYes, mbNo], 0) = mrNo then
        exit;
    end
  else
    begin
      if MessageDlg ('Deseja eliminar os lançamentos escaneados?'
                     , mtWarning,[mbYes, mbNo], 0) = mrNo then
        exit;
    end;
...
begin
  ...
  if MessageDlg ('Confirma gravação de demarcações?'
                  , mtWarning,[mbYes, mbNo], 0) = mrNo then exit;
  ...
  try
    ...
    on E : Exception do
      begin
        ShowMessage('Erro ao atualizar demarcações em piquetes(Areas_tb) - '+E.Message);
  ...
  showmessage('Foram incluidas ' + vContador.ToString + ' coordenadas xy da tela e ' +
              vNaoIncluidas.ToString + ' não incluídas');
  Screen.Cursor := crDefault;
  vDemarcou := false;
  imgPrincipal.Picture := imgSalva.Picture;
  btdemarcaPiquete.Color := clWhite;
...
begin
  ...
  if pnMedicoesEscaneamentos.Visible and pnPiquete.Visible then
    begin
      showmessage('As janelas de Medições Escaneamentos e de Piquetes não ' +
  ...
  if length(trim(mkeYdesenho.Text)) = 0 then
    begin
      showmessage('Ydesenho não informado');
      mkeYdesenho.SetFocus;
      exit;
    end;
  ...
  if length(trim(mkeYajuste.Text)) = 0 then
    begin
      showmessage('Yajuste não informado');
      mkeYajuste.SetFocus;
      exit;
    end;
  ...
  if length(trim(mkeXdesenho.Text)) = 0 then
    begin
      showmessage('Xdesenho não informado');
  ...
  if length(trim(mkeXajuste.Text)) = 0 then
    begin
      showmessage('Xajuste não informado');
      mkeXajuste.SetFocus;
      exit;
    end;
...
begin
  ...
  if length(trim(edPixe1Lat.Text)) = 0 then
    begin
      showmessage('pixel1 da latitude não informado');
      edCoordenada1Lat.SetFocus;
      exit;
    end;
  ...
  if length(trim(edPixe2Lat.Text)) = 0 then
    begin
      showmessage('pixel2 da latitude não informado');
      edCoordenada2Lat.SetFocus;
      exit;
    end;
  ...
  if length(trim(edPixe1Long.Text)) = 0 then
    begin
      showmessage('pixel1 da longitude não informado');
      edCoordenada1Long.SetFocus;
      exit;
    end;
  ...
  if length(trim(edPixe2Long.Text)) = 0 then
    begin
      showmessage('pixel2 da longitude não informado');
      edCoordenada2Long.SetFocus;
      exit;
    end;
...
begin
  ...
  if length(trim(edCoorPixLat.Text)) = 0 then
    begin
      showmessage('Relação coordenada/pixel não calculada');
      exit;
    end;
  ...
  showmessage('Relação coordenada/pixel alterada com sucesso');
...
begin
  ...
  if MessageDlg ('Deseja escolher mais um arquivo de medição?'
                  , mtWarning,[mbYes, mbNo], 0) = mrYes then
    begin
      CapturaPath;
      frmScannForr.Refresh;
    ...
    if mmRodape.Lines.Count = 0 then
      begin
        showmessage('Falta escolher o arquivo de medição');
        exit;
      end
      ...
      if vTemErro then
        begin
          showmessage('Erro no conteúdo do arquivo: ' + vpath);
          pnCalibra.Visible := true;
          frmScannForr.Refresh;
        end
      else
        begin
          ...
          showmessage('****** ATENÇÃO! É necessário executar Gera Pontos Tela ' +
                      'para gerar as coordenadas xy referente às coordenadas geográficas ' +
                      'escaneadas.');
        end;
...
begin
  ...
  OpenDialog1.Title := 'Selecione o caminho do arquivo de Medição';
  ...
  begin
    showmessage('O arquivo ' + path + ' não existe');
    exit;
  end;
...
begin
  ...
  begin
    showmessage('Existem ' + linhasSemOrigem.ToString +
                ' linhas não identificadas nem como calibragem nem como escaneamento');
    exit;
  end;

  if (calibragem = 0) or (fimCalibragem = 0) or (linhasCalibragem = 0) then
  begin
    showmessage('A calibragem não foi feita');
    exit;
  end;

  if (calibragem > fimCalibragem) then
  begin
    showmessage('Existe calibragem que não foi encerrada');
    exit;
  end;

  if (calibragem < fimCalibragem) then
  begin
    showmessage('Existe calibragem que não foi iniciada');
    exit;
  end;

  if (linhasEscaneamento = 0) then
  begin
    showmessage('O escaneamento não foi feito');
    exit;
  end;

  if setup > 1 then
  begin
    if MessageDlg ('***** ATENÇÃO - O equipamento foi reiniciado ' + setup.ToString +
                   ' vezes - CONTINUA A IMPORTAÇÃO?'
                    , mtWarning,[mbYes, mbNo], 0) = mrNo then
      begin
        vTemErro := false;
        exit;
  ...
  begin
    if MessageDlg ('***** ATENÇÃO - A calibragem foi feita ' + calibragem.ToString +
                   ' vezes - CONTINUA A IMPORTAÇÃO?'
                    , mtWarning,[mbYes, mbNo], 0) = mrNo then
      begin
        vTemErro := false;
  ...
  if qryMedicoes_escaneamentos.RecordCount = 0 then
    ...
    begin
      if vi = 0 then
      if MessageDlg ('***** ATENÇÃO - Os dados de escaneamento dessa Medição: "'+
                     qryMedicoes_capa_scann.FieldByName('descricao_medicao').AsString +
                     '" já foram incluidos e serão apagados, PODE APAGÁ-LOS?'
                      , mtWarning,[mbYes, mbNo], 0) = mrNo then
        begin
          vTemErro := false;
          exit;
      ...
      if vi = 0 then
      if MessageDlg ('***** ATENÇÃO - Se você não tiver o arquivo de escaneamento ' +
                     ' não conseguirá recuperar esses dados, DESEJA CONTINUAR?'
                      , mtWarning,[mbYes, mbNo], 0) = mrNo then
        begin
          vTemErro := false;
          exit;
...
begin
  ...
  try
    ...
    on E : Exception do
      begin
        ShowMessage('Erro ao atualizar demarcações em piquetes(limpa - Areas_tb) - '+E.Message);
        qryAreas.CancelUpdates;
        exit;
      end;
  end;
...
begin
  ...
  if length(dbeNomeAreaGeral.Text) = 0 then
    begin
      showmessage('informe o nome da área de piquetes');
      dbeNomeAreaGeral.SetFocus;
      abort;
    end;
  ...
  if length(dbePathImagemAreaGeral.Text) = 0 then
    begin
      showmessage('informe o caminho onde está a imagem da área de piquetes');
  ...
  if length(dblVariedades.Text) = 0 then
    begin
      showmessage('informe a variedade da forragem utilizada nessa área');
      abort;
    end;
...
begin
  ...
  if length(dbeDescricao_medicao.Text) = 0 then
    begin
      showmessage('informe a descrição da medição');
      dbeDescricao_medicao.SetFocus;
      abort;
    end;
  ...
  if length(dbeOperador.Text) = 0 then
    begin
      showmessage('informe o operador da medição');
  ...
  try
    ...
    begin
      showmessage('data da medição inválida');
      mkeData.SetFocus;
      abort;
    end;
...
begin
  ...
  if qryUnidades.FieldByName('unidade_padrao').AsBoolean then
    begin
      showmessage('Essa unidade é padrão do sistema, não pode ser alterada nem apagada');
...
begin
  ...
  if qryUnidades_proporcionais.FieldByName('relacao_padrao').AsBoolean then
    begin
      showmessage('Essa relação de unidade é padrão do sistema, não pode ser alterada nem apagada');
...
begin
  ...
  if length(dbeDescricao_unidade_proporcional.Text) = 0 then
    begin
      showmessage('informe a descrição da unidade proporcional');
      dbeDescricao_unidade_proporcional.SetFocus;
      abort;
    end;
...
begin
  ...
  if qryVariedades.FieldByName('variedade_padrao').AsBoolean then
    begin
      showmessage('Essa variedade é padrão do sistema, não pode ser alterada nem apagada');
...
begin
  ...
  if length(dbeAltura_minima_pastejo.Text) = 0 then
    begin
      showmessage('informe a altura mínima possível para o pastejo em cm, sem casa decimal');
  ...
  if length(dbeAltura_maxima_crescimento.Text) = 0 then
    begin
      showmessage('informe a altura máxima de crescimento viável para a planta');
...
procedure TfrmScannforr.LimpaRelaçãoCoordenadasPixel;
begin
...
begin
  ...
  if MessageDlg ('***** ATENÇÃO - Deseja sair da aplicação? Salve seus dados.'
                 , mtWarning,[mbYes, mbNo], 0) = mrNo then exit;
...
begin
  ...
  if qryGeral2.RecordCount > 0 then
    begin
      if MessageDlg ('Os resultados já foram gerados, Tem certeza que '+
                     'deseja continuar e eliminar os lançamentos escaneados '+
                     'com essa nova importação?'
                     , mtWarning,[mbYes, mbNo], 0) = mrNo then
        exit;
    end;
