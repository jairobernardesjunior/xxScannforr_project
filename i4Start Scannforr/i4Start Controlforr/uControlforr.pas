...
type
  TfrmControlforr = class(TForm)
    ...
    procedure GravaPosiçãoDiariaAnimais_embrapa;
    procedure GravaPosiçãoDiariaAnimais_semRaca;
...
begin
  ...
  if qryLinhaTempo.RecordCount = 0 then
    begin
      showmessage('ainda não foi feito nenhuma pesquisa de ciclo');
      exit;
    end;
...
begin
  ...
  if qryMovto_animais.FieldByName('saida').AsBoolean then
    begin
      showmessage('esse movimento é de saída, escolha um movimento de entrada');
      exit;
    end;

  if qryMovto_animais.FieldByName('cancelado').AsBoolean then
    begin
      showmessage('esse movimento está inativo');
      exit;
    end;
  ...
  if vDataFimCiclo <> '31/12/3000' then
    begin
      showmessage('o ciclo já está fechado para esse movimento, ' +
                  'não é permitido fazer saída nem transferência');
      exit;
    end;
  ...
  if (copy(vDataInicioCiclo, 1, 5) = '!@#$%') then
    begin
      showmessage('não foi encontrado a data de início do ciclo');
      exit;
    end;
...
status;
  ...
  if vsaldoAnimais = 0 then
    begin
      showmessage('já foram feitas todas as saídas de animais para essa entrada');
      exit;
    end
  else
  if vsaldoAnimais < 0 then
    begin
      showmessage('***** Atenção existem mais saídas do que a qtde dessa entrada' +
                  ' - saídas: ' + xqtde.ToString + ' - entradas: ' +
                  qryMovto_Animais.FieldByName('qtde_animais').AsString + ' - ' +
                  'alguns movimentos de saída precisam ser inativados e ou refeitos');
      exit;
    end;
...
begin
  ...
  if (strTOdateTime(mkeDataInicioMonta.text) < strTOdateTime(vDataInicioCiclo)) or
    ...
    begin
      showmessage('data para montar linha do tempo menor que o início do ciclo ou ' +
                  '>= que o fim do ciclo');
      mkeDataInicioMonta.SetFocus;
      exit;
    end;
  ...
  if qryLinhaTempo.Eof then
    begin
      showmessage('não foi encontrada nenhuma data de movimentação maior ou igual a ' +
...
begin
  ...
  if qryLinhaTempo.RecordCount > 0 then
    ...
    begin
      showmessage('faça uma montagem da linha do tempo');
      exit;
    end;
...
' order by 1, 2, 3';
  ...
  for i := 1 to qryGeral.RecordCount do
    begin
      ...
      mmaux.Lines.Add('      Altura Média da Forragem: ' +
                      formatCurr('###,###,##0',
...
{          if xsaldoQtde >= 0 then
      ...
      if qryGeral.FieldByName('entradaSaida').AsInteger = 1 then
        ...
        begin
          mmaux.Lines.Add('    ' + xprevistoRealizado + ' - Saída: ' +
                          qryGeral.FieldByName('qtde').AsString +
                          ' - ' +
                          trim(qryGeral.FieldByName('descricao_tipo').AsString) +
                          ' - '  + trim(qryGeral.FieldByName('nome_area').AsString));
...
begin
  ...
  mmaux.Lines.Clear;
  mmaux.Lines.Add('');
  mmaux.Lines.Add(' APURAÇÃO ANALÍTICA DO CICLO');
  mmaux.Lines.Add('');
...
begin
  ...
  if MessageDlg ('***** ATENÇÃO - Deseja limpar a tela de gráficos'
...
begin
  ...
  if not cbMovtoSuprealizado.Checked then
    begin
      if MessageDlg ('***** ATENÇÃO - Deseja marcar/desmarcar esse movimento?'
                    , mtWarning,[mbYes, mbNo], 0) = mrNo then
                    begin
                      qryMovto_suplementos.CancelUpdates;
                      exit;
  ...
  if cbMovtoaniCancelado.Checked then
    ...
    '   and cancelado <> true ';
      ...
      if qryGeral.RecordCount > 0 then
        begin
          qryMovto_animais.CancelUpdates;
          showmessage('existe(m) saída(s) e/ou transferência(s) ativas para essa '+
                      'entrada');
          exit;
        end;
...
begin
  ...
  if vDataFimCiclo <> '31/12/3000' then
    begin
      showmessage('o ciclo já está fechado para esse movimento, ' +
                  'não é permitido marcar/desmarcar');
      qryMovto_animais.CancelUpdates;
      exit;
    end;
  ...
  if key = 13 then
    begin
      if length(ednomeArquivoGrafico.Text) = 0 then
        begin
          showmessage('informe o nome do arquivo para salvar o gráfico');
          ednomeArquivoGrafico.SetFocus;
          exit;
        end;
...
begin
  ...
  if qryConsumo_raca.RecordCount = 0 then
    begin
      showmessage('a tabela consumo_raca_tb está vazia, a linha do tempo não funcionará '+
                  'corretamente!');
      exit;
    end;
...
' where id_raca is null or id_raca = 0';
  ...
  if qryGeral2.RecordCount > 20 then
    begin
      showmessage('tipos de animais cadastrados maior que 20, tente usar até 20 tipos');
      exit;
    end;
...
begin
  ...
  if length(dbeDescricao_movto_animais.Text) = 0 then
    begin
      showmessage('informe a descrição do movimento de animais');
  ...
  if ( (length(dbeGanhoDepesoDiakg.Text) = 0) or (strTocurr(dbeGanhoDepesoDiakg.Text) <= 0) ) then
    begin
      showmessage('informe o ganho médio de peso por dia do animal');
...
{  if qryTipo_Animalcb.FieldByName('id_raca').AsCurrency > 0 then
  ...
  if strTocurr(dbeGanhoDepesoDiakg.Text) > 0 then
    begin
      showmessage('ganho médio de peso por dia tem de ficar zerado para esse tipo de animal');
      dbeGanhoDepesoDiakg.SetFocus;
      exit;
    end;
  ...
  if (cbsaida.Checked) then
    begin
      showmessage('O movimento de saída de animais deve ser feito na Transferência' +
                  ' de Piquete, botão Transf Animais');
      exit;
    end;
...
'     0 as alt_min_pastejo,' +  // altura de saída
...
begin
  ...
  '     mc.altura_minima_pastejo as alt_min_pastejo,' +  // altura de saída
...
begin
  ...
  if qryCrescimentoForragem.RecordCount = 0 then
    begin
      showmessage('não foi encontrado nenhum registro para a montagem ' +
...
begin
  ...
  if not cbEventoRealizado.Checked then
    begin
      if MessageDlg ('***** ATENÇÃO - Deseja marcar/desmarcar esse evento?'
                    , mtWarning,[mbYes, mbNo], 0) = mrNo then
                    begin
                      qryeventos.CancelUpdates;
                      exit;
...
begin
  ...
  if qryLinhaTempo.FieldByName('ordem').AsInteger = 2 then
    begin
      ...
      if qryLinhaTempo.FieldByName('entrada').AsBoolean then
        ...
        mmAux.Lines.Add('Saída do piquete');

      mmAux.Lines.Add('');

      if qryLinhaTempo.FieldByName('femea').AsBoolean then
        mmAux.Lines.Add('Animal fêmea')
      else
      ...
      if vsaldoAnimais = 0 then
        begin
          mmAux.Lines.Add('***** Não tem mais animais nesse piquete');
          mmAux.Lines.Add(' ');
          vcorDemo := 'rosa';
        end
      else
  ...
  if qryLinhaTempo.FieldByName('ordem').AsInteger = 9 then
    begin
      ...
      mmAux.Lines.Add(
        'Altura média medida da forragem: ' +
      ...
      mmAux.Lines.Add(
        'Altura mínima de pastejo (altura de saída): ' +
      ...
      if xAlturaCamadaPastejavel = 0 then
        begin
          mmAux.Lines.Add(
            '***** A altura da camada pastejável está igual a zero, a forragem ' +
            'está na altura mínima de pastejo: ' +
            formatCurr('##0', qryLinhaTempo.FieldByName('alt_min_pastejo').AsCurrency) +
            ' cm'
            );
          mmAux.Lines.Add(' ');
        ...
        begin
          ...
          mmAux.Lines.Add(
            '***** A altura da forragem está abaixo da altura viável para pastejo em: ' +
            formatCurr('##0', xAlturaCamadaPastejavel) +
            ' cm'
            );
          mmAux.Lines.Add(' ');
      ...
      // altura máxima de crescimento  // altura de entrada
      ...
      if xAlturaMaximaCrescimento <= 0 then
        ...
        begin
          mmAux.Lines.Add(
            '***** A altura da forragem excedeu a altura máxima viável de crescimento em: ' +
      ...
      if vkg_ms_ha_dia = 0 then
        begin
          showmessage('data: ' + DateTimeTOstr(vdataScala) +
                      ' não encontrada na recuperação de estação');
          abort;
        end;
...
begin
  ...
  qryGeral2.Params[6].AsString := ''; //descrição
        ...
        begin
          ...
          //if vQtdeAnimalAtual_Embrapa[i, ii, iii] > 0 then
            begin
              //if id > 1 then
                begin
                  GravaPosiçãoDiariaAnimais_embrapa;
                  vdataScala := incDay(vdataScala, 1);
                  if Id <> xdias then
                    CalculaPesoConsumo_embrapa;
                  if vtemerro then
...
begin
  ...
  if i = 0 then
    begin
      showmessage('peso vivo atual não encontrado na tabela da embrapa, peso = ' +
        ...
        begin
          ...
          //if vQtdeAnimalAtual_semRaca[i] > 0 then
            begin
              //if id > 1 then
                begin
                  GravaPosiçãoDiariaAnimais_semRaca;
                  vdataScala := incDay(vdataScala, 1);
                  if Id <> xdias then
                    CalculaPesoConsumo_semRaca;
                  if vtemerro then
...
procedure TfrmControlforr.GravaPosiçãoDiariaAnimais_embrapa;
begin
...
procedure TfrmControlforr.GravaPosiçãoDiariaAnimais_semRaca;
begin
  ...
  qryGeral2.Params[4].AsString := 'Consumo-Peso Animal/dia semRaça';
  qryGeral2.Params[5].AsString := dateTimeTOstr(vdataScala);
  ...
  try
    ...
    on E : Exception do
    begin
      ShowMessage('Erro ao incluir movimento diario de animais semraça na tab linha_dotempo_tb : '+E.Message);
      abort;
    end;
  end;
...
' order by m.seq_movto_animais, m1.seq_movto_animais';
  ...
  mmaux.Lines.Clear;
  mmaux.Lines.Add('');
  mmaux.Lines.Add(' Entradas - Saídas - Transferências');
...
begin
  ...
  if length(dbeTganhoDepesoDiakg.Text) = 0 then
    begin
      showmessage('informe o ganho médio de peso por dia em kg do tipo de animal, por cabeça');
...
begin
  ...
  if (dcbSaida.Checked) and
    ...
    begin
      showmessage('para saída de forragem use somente unidade de kg');
      abort;
    end;
...
begin
  ...
  if length(dbeDescricao_movto_suplemento.Text) = 0 then
    begin
      showmessage('informe a descrição do movimento de suplemento');
      dbeDescricao_movto_suplemento.SetFocus;
      abort;
    end;
  ...
  if ( (length(dbeqtdeDiasUsado.Text) = 0) or (strTocurr(dbeqtdeDiasUsado.Text) <= 0) ) then
    begin
      showmessage('informe em quantos dias a quantidade do suprimento será distribuida e utilizada');
...
begin
  ...
  try
    ...
    showmessage('data de saída inválida');
  ...
  try
    ...
    showmessage('peso de saída inválido');
  ...
  if xtestaData <
    ...
    begin
      showmessage('data de saída menor que a data de entrada dos animais');
      mkedataSaida.SetFocus;
      exit
    end;
  ...
  if vsaldoAnimais = 0 then
    begin
      showmessage('não existe mais animais a ser transferido, saldo = ' +
                  vsaldoanimais.ToString );
  ...
  if vsaldoAnimais < strTOint(trim(mkeQtdeSaida.Text)) then
    begin
      showmessage('qtde de saída maior que o saldo restante da entrada de animais = ' +
                  vsaldoanimais.ToString );
      mkeqtdeSaida.SetFocus;
      exit
    end;
  ...
  if strTOint(trim(mkeQtdeSaida.Text)) = 0 then
    begin
      showmessage('quantidade de saída de animais está zerada');
      mkeqtdeSaida.SetFocus;
      exit
    end;
  ...
  if xtestaPeso <
    ...
    begin
      showmessage('peso de saída menor que o peso de entrada');
      mkepesoSaida.SetFocus;
      exit
    end;

  if trim(mkeDataEntrada.Text) = '00/00/0000' then
    if MessageDlg ('A data de entrada não está informada, não será feito nenhuma' +
                   ' transferência, prossegue?'
                 , mtWarning,[mbYes, mbNo], 0) = mrNo then
      begin
  ...
  if trim(mkeDataEntrada.Text) <> '00/00/0000' then
    begin
      ...
      if strTOdateTIme(mkeDataEntrada.Text) > strToDateTime(mkeDataSaida.Text) then
        ...
        begin
          showmessage('a data de entrada na transferência tem de ser maior que a data ' +
                      'de saída');
          mkedataEntrada.SetFocus;
          exit
        end;
      ...
      if lbpiquete_transfere.Caption = '...' then
        begin
          showmessage('piquete de transferência não informado, clique na tabela de piquetes ' +
                      'para selecionar');
          exit
        end;
      ...
      if lbtipo_animal_transfere.Caption = '...' then
        begin
          showmessage('tipo de animal não informado, clique na tabela de tipo de animais ' +
                      'para selecionar');
          exit
        end;
      ...
      if qryMovto_animais.FieldByName('seq_area').AsInteger =
        ...
        if qryMovto_animais.FieldByName('seq_tipo_animal').AsInteger =
          ...
          begin
            showmessage('o piquete de transferência é o mesmo da saída, escolha outro');
            exit
          end;

      // PROCURA DATA DE FIM E INÍCIO DO CICLO
  ...
  if trim(mkeDataEntrada.Text) = '00/00/0000' then
    begin
      qryGeral2.Params[11].AsString :=     //descrição do movimento animal
                  'saída piquete ' +
  ...
  if trim(mkeDataEntrada.Text) <> '00/00/0000' then
    begin
      ...
      try
        ...
        on E : Exception do
        begin
          ShowMessage('Erro ao incluir movimento animais transferência em movto_animais_tb : '+E.Message);
          exit;
        end;
      end;
...
begin
  ...
  if button = mbright then
    begin
      OpenDialog1.Title := 'Selecione o caminho do arquivo de gráfico';
  ...
  if vTemNotificacao[xidx] then
    begin
      ...
      if vforragemSaldo_cm[xidx] >= trunc(xlimiteMaisPerc) then
        begin
          mmNote.Lines.Add(
            '***** Saldo de forragem está acima da altura de entrada padrão em ' +
            formatCurr('###,###,###,##0.00',
                qryPadrao_sistema.
          ...
          mmNote.Lines.Add('');
          mmNote.Lines.Add(
            'Altura de entrada padrão + percentual limite: ' +
      ...
      if vforragemSaldo_cm[xidx] >=
        ...
        begin
          mmNote.Lines.Add(
            '***** Atenção o saldo de forragem atingiu a altura de entrada');
          mmNote.Lines.Add('');
        end
      ...
      if vforragemSaldo_cm[xidx] <=
        ...
        begin
          mmNote.Lines.Add(
            '***** Atenção o saldo de forragem atingiu a altura de saída');
          mmNote.Lines.Add('');
        end;
